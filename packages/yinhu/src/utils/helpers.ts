 ...
 * set on the host. For example, the inner input in `yh-input` should inherit
 * the `title` attribute that developers set directly on `yh-input`. This
...
export const findItemLabel = (componentEl: HTMLElement): any | null => {
  const itemEl = componentEl.closest('yh-item');
  if (itemEl) {
    return itemEl.querySelector('yh-label');
  }
  return null;
};
...
export const getAriaLabel = (componentEl: HTMLElement, inputId: string): { label: Element | null; labelId: string; labelText: string | null | undefined } => {
  ...
  // we should use that instead of looking for an yh-label
